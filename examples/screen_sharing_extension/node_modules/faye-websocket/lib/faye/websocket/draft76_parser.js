...
import Draft75Parser from './draft75_parser';
var Draft76Parser = function() { Draft75Parser.apply(this, arguments) };

var bridge = () => {};
bridge.prototype = Draft75Parser.prototype;
Draft76Parser.prototype = new bridge();

var numberFromKey = key => parseInt(key.match(/[0-9]/g).join(''), 10);

var spacesInKey = key => key.match(/ /g).length;

var bigEndian = number => {
  var string = '';
  [24,16,8,0].forEach(offset => {
    string += String.fromCharCode(number >> offset & 0xFF);
  });
  return string;
};

Draft76Parser.prototype.getVersion = () => 'hixie-76';
