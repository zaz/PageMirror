...
import Handshake from './hybi_parser/handshake';
import Reader from './hybi_parser/stream_reader';
...
HybiParser.mask = (payload, mask, offset) => {
...
var instance = {
  ...
  handshakeResponse() {
    ...
    if (protos !== undefined && supported !== undefined) {
      ...
      proto = protos.filter(p => supported.indexOf(p) >= 0)[0];
      if (proto) {
        this.protocol = proto;
  ...
  _parseOpcode(data) {
    var rsvs = [this.RSV1, this.RSV2, this.RSV3].filter(rsv => (data & rsv) === rsv, this);
  ...
  frame(data, type, code) {
    ...
    if (length <= 125) {
      ...
      frame[1] = masked | 127;
      frame[2] = Math.floor(length / (2 ** 56)) & BYTE;
      frame[3] = Math.floor(length / (2 ** 48)) & BYTE;
      frame[4] = Math.floor(length / (2 ** 40)) & BYTE;
      frame[5] = Math.floor(length / (2 ** 32)) & BYTE;
      frame[6] = Math.floor(length / (2 ** 24)) & BYTE;
      frame[7] = Math.floor(length / (2 ** 16)) & BYTE;
      frame[8] = Math.floor(length / (2 ** 8))  & BYTE;
      frame[9] = length & BYTE;
    }
  ...
  close(code, reason, callback, context) {
